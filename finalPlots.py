...
binsred = np.linspace(-2.4,2.4, 24+1)
binsredC = (binsred[:-1] + binsred[1:]) / 2

fmc = ROOT.TFile.Open("outClosureReco.root")
fmc_layer = ROOT.TFile.Open("calibrationJMC_smeared_001_constant_abssigma.root")
fd = ROOT.TFile.Open("outClosureTruth.root")
fd_layer = ROOT.TFile.Open("calibrationJMC.root")
# unbinned fit
with np.load('/scratchnvme/emanca/scipy-MuCal/unbinnedfitglobalitercorscale_zbs.npz') as f:
      xs = f["xs"]
      xerrs = f["xerrs"]
with np.load('unbinnedfitglobalitercorscale_jpsi.npz') as f:
      xs_j = f["xs"]
      xerrs_j = f["xerrs"]
...
derrmc = np.sqrt(np.array([fmc.Get('d').GetSumw2()[i] for i in range(fmc.Get('d').GetSumw2().GetSize())])[1:-1])
...
derrd = np.sqrt(np.array([fd.Get('d').GetSumw2()[i] for i in range(fd.Get('d').GetSumw2().GetSize())])[1:-1])
...
# hep.histplot(hAmc,bins,histtype = 'errorbar', yerr=Aerrmc, ax=ax1, label = ["$J/\psi$ gen spectra"])
# hep.histplot(hAmc_layer,bins,histtype = 'errorbar', yerr=Aerrmc_layer, ax=ax1, label = ["$J/\psi$ smeared spectra constant"],color='orange')
# hep.histplot(hAd,bins,histtype = 'errorbar', yerr=Aerrd, ax=ax1, label = ["$J/\psi$ smeared spectra"], color='red')
# hep.histplot(hAd_layer,bins,histtype = 'errorbar', yerr=Aerrd_layer, ax=ax1, label = ["smeared mass"], color='green')
...
# ax1.legend(loc='upper right', bbox_to_anchor=(0.5, 1.05), frameon=False)
ax1.set_xlabel('$\eta$')
plt.tight_layout()
...
# hep.histplot(hemc,bins,histtype = 'errorbar', yerr=eerrmc, ax=ax1, label = ["$J/\psi$ gen spectra"])
# hep.histplot(hemc_layer,bins,histtype = 'errorbar', yerr=eerrmc_layer, ax=ax1, label = ["$J/\psi$ smeared spectra constant"],color='orange')
# hep.histplot(hed,bins,histtype = 'errorbar', yerr=eerrd, ax=ax1, label = ["$J/\psi$ smeared spectra"], color='red')
...
hep.cms.text('work in progress', loc=1, ax=ax1)
ax1.set_title("a", fontsize=18)
hep.histplot(hamc,binsred,histtype = 'errorbar', yerr=aerrmc, ax=ax1, label = ["$J/\psi$ + $Z$ mc"])
hep.histplot(had,binsred,histtype = 'errorbar', yerr=aerrd, ax=ax1, label = ["$J/\psi$ + $Z$ mc truth"], color='red')
# hep.histplot(xs[...,3],bins,histtype = 'errorbar', yerr=xerrs[...,3], ax=ax1, label = ["mc truth (z)"], color='green')
# hep.histplot(xs_j[...,3],bins,histtype = 'errorbar', yerr=xerrs_j[...,3], ax=ax1, label = ["single muon"], color='orange') # hep.histplot(xs_single[...,3],bins,histtype = 'errorbar', yerr=xerrs[...,3], ax=ax1, label = ["mc truth"], color='magenta')
# ax1.set_ylim([np.min(hmc)-0.0005, np.max(hmc)+0.0005])
ax1.legend(loc='upper right', frameon=True)
...
hep.histplot(hbmc,binsred,histtype = 'errorbar', yerr=berrmc, ax=ax1, label = ["$J/\psi$ + $Z$ mc"])
hep.histplot(hbd,binsred,histtype = 'errorbar', yerr=berrd, ax=ax1, label = ["$J/\psi$ + $Z$ mc truth"], color='red')
...
hep.histplot(hcmc,binsred,histtype = 'errorbar', yerr=cerrmc, ax=ax1, label = ["$J/\psi$ + $Z$ mc"])
hep.histplot(hcd,binsred,histtype = 'errorbar', yerr=cerrd, ax=ax1, label = ["$J/\psi$ + $Z$ mc truth"], color='red')
...
hep.histplot(hdmc,binsred,histtype = 'errorbar', yerr=derrmc, ax=ax1, label = ["$J/\psi$ + $Z$ mc"])
hep.histplot(hdd,binsred,histtype = 'errorbar', yerr=derrd, ax=ax1, label = ["$J/\psi$ + $Z$ mc truth"], color='red')
ax1.legend(loc='upper right', frameon=True)
# ax1.set_ylabel('number of events')
ax1.set_xlabel('$\eta$')
plt.tight_layout()
...
# closure of scale
filesJ = [h5py.File('fitsJMC.hdf5', mode='r'), h5py.File('fitsJDATA.hdf5', mode='r')]
...
etasZ = np.linspace(-2.4, 2.4, 49, dtype='float64')
etasZC = (etasZ[:-1] + etasZ[1:]) / 2

scalesEtaJmc_red = bin_ndarray(np.array(scalesEtaJmc),(48,),'mean')
scalesEtaJdata_red = bin_ndarray(np.array(scalesEtaJdata),(48,),'mean')
scalesEtaZmc_red = bin_ndarray(np.array(scalesEtaZmc),(48,),'mean')
scalesEtaZdata_red = bin_ndarray(np.array(scalesEtaZdata),(48,),'mean')

scalesErrEtaJmc_red = np.sqrt(1./bin_ndarray(np.reciprocal(np.square(np.array(scalesErrEtaJmc))),(48,),'sum'))
scalesErrEtaJdata_red = np.sqrt(1./bin_ndarray(np.reciprocal(np.square(np.array(scalesErrEtaJdata))),(48,),'sum'))
scalesErrEtaZmc_red = np.sqrt(1./bin_ndarray(np.reciprocal(np.square(np.array(scalesErrEtaZmc))),(48,),'sum'))
scalesErrEtaZdata_red = np.sqrt(1./bin_ndarray(np.reciprocal(np.square(np.array(scalesErrEtaZdata))),(48,),'sum'))
...
# ax1.errorbar(etasC, 1.-np.array(scalesEtaJdata_red),yerr=scalesErrEtaJdata_red, marker="v", label = '$J/\psi$ data',fmt='v')
ax1.errorbar(etasC, 1.-np.array(scalesEtaJmc_red),yerr=scalesErrEtaJmc_red, marker="v", label = '$J/\psi$ mc',fmt='v')
...
fig, ax1 = plt.subplots()
hep.cms.text('work in progress', loc=1, ax=ax1)
# ax1.errorbar(ptsC[:8], 1.-np.array(scalesJdata),yerr=np.array(scalesErrJdata), marker="v", label = '$J/\psi$ data',fmt='v')
ax1.errorbar(ptsC[:8], 1.-np.array(scalesJmc),yerr=np.array(scalesErrJmc), marker="v", label = '$J/\psi$ mc',fmt='v')
...
fig, ax1 = plt.subplots()
hep.cms.text('work in progress', loc=1, ax=ax1)
ax1.errorbar(ptsC[:8], slopeJdPatched[24,23,:,4], marker="v", label = 'slope $J/\psi$ data',fmt='v')
plt.savefig('FinalPlots/slope_pt.png')
