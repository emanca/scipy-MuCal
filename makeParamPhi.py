...
f = ROOT.TFile("outClosureTruthPhi.root", 'recreate')
f.cd()
hA.Write()
he.Write()
hM.Write()
...
scaleModel = scale(A,e,M,ks,qs)
scaleModelMinus = scale(A,e,M,np.flip(ks,axis=0),qs)
...
for i in range(nEtaBins):
    ...
    plt.tight_layout()
    plt.savefig('PlotsScaleTruthAfter/scalephi_eta{}.png'.format(round(etasC[i],2)))
