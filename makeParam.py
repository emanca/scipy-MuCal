...
def scalesigma(parms, qs, ks):
    ...
    b = parms[..., 4, np.newaxis, np.newaxis]
...
xsigma = 0.1*np.ones((nEtaBins,2,nPtBinsJ),dtype=np.float64)
...
xsigma = 1.*np.ones((nEtaBins,2,nPtBinsZ),dtype=np.float64)
xbinned = np.stack((xmu,xsigma),axis=-1)
...
# parmsmodel = np.concatenate([parmscale, parmsigma0, parmsigma1,parmsigma1,parmsigma0], axis=-1)
...
# ha = array2hist(-1*A, hA, Aerr)
# he = array2hist(-1*e, he, eerr)
...
# f = ROOT.TFile("outClosureTruth.root", 'recreate')
# f.cd()
# hA.Write()
# he.Write()
# hM.Write()
...
fileOut = "scalemctruth_afterNoAlign.hdf5"
...
for i in range(nEtaBins):
    ...
    #     transform=ax1.transAxes,
    #     color='black', fontsize=10)
    binstot = np.concatenate((-1*binCentersZfull[i,...,4],-1*binCentersJfull[i,...,4],binCentersJfull[i,...,4],binCentersZfull[i,...,4]))
